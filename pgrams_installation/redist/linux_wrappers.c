...
REGISTER_FUNC_MOD kp_register_mod_func_t pgrams_register_kp_module_func;
#else
REGISTER_FUNC_MOD kp_register_mod_func_t windrvr1630_register_kp_module_func;
#endif

#if defined(WINDRIVER_KERNEL)
    #define EXPORT_SYMTAB
typedef ssize_t(*LINUX_STORE_FUNC)(struct device_driver *, const char *,
    size_t);
#endif
    ...
    _u64 *phys)
{
    ...
    int gfp = ((flag & DMA) ? GFP_DMA : 0) | __GFP_NOWARN | GFP_KERNEL;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6,2,0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5,14,0)) || \
    defined(__aarch64__)
    unsigned long attrs = DMA_ATTR_FORCE_CONTIGUOUS | DMA_ATTR_NO_WARN;
#endif
    ...
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5,14,0)) || \
    defined(__aarch64__)
        res = dma_alloc_attrs(&hwdev->dev, size, &dma_handle, gfp, attrs);
#else
...
void LINUX_dma_contig_free(void *pdev, void *va, unsigned int size, _u64 phys)
{
    ...
    if (!hwdev)
        ...
        (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5,14,0)) || \
        defined(__aarch64__)
        dma_free_attrs(&hwdev->dev, size, va, phys, DMA_ATTR_FORCE_CONTIGUOUS);
    #else
...
static inline int uncached_access(struct file *file, unsigned long addr)
{
	...
	(defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5,14,0))
        ...
        {
            LINUX_mmap_private *private_data =
                (LINUX_mmap_private *)file->private_data;
        ...
        {
            /* Map I/O buffer - Physical address mapping */
...
#else
/* A version of wd_mmap which is works on ARM64, but could also work
   work on other platforms but with possibly reduced performance */
...
static int wd_mmap(struct file *file, struct vm_area_struct *vma)
    ...
    #if LINUX_VERSION_CODE >= KERNEL_VERSION(6,3,0)
        vm_flags_set(vma, VM_DONTEXPAND | VM_DONT_DUMP);
    #elif LINUX_VERSION_CODE >= KERNEL_VERSION(3,7,0)
        vma->vm_flags |= VM_DONTEXPAND | VM_DONTDUMP; /* don't swap out */
    #else
        vma->vm_flags |= VM_RESERVED; /* don't swap out */
...
const char *LINUX_get_driver_name(void)
{
    ...
    {
    #if defined(WD_DRIVER_NAME_CHANGE)
        return pgrams_register_kp_module_func;
    #else
        return windrvr1630_register_kp_module_func;
    #endif
    }
    ...
    {
    #if defined(WD_DRIVER_NAME_CHANGE)
        unix_get_register_kp_module(&(pgrams_register_kp_module_func));
#else
        unix_get_register_kp_module(&(windrvr1630_register_kp_module_func));
#endif
    }
#endif
...
#if defined(WD_DRIVER_NAME_CHANGE)
EXPORT_SYMBOL(pgrams_register_kp_module_func);
#else
EXPORT_SYMBOL(windrvr1630_register_kp_module_func);
#endif
...
#if defined(WD_DRIVER_NAME_CHANGE)
    #define WD_PCI_DRIVER_NAME "pgrams_pci"
#else
    #define WD_PCI_DRIVER_NAME "windrvr1630_pci"
#endif
    ...
    const struct pci_device_id *id)
{
    ...
    if (LINUX_is_dev_in_pci_table(dev))
    {
        /* Take ownership */
        ret = 0;
        goto Exit;
    ...
    /* Look at the dynamic ids */
    spin_lock(&generic_pci_driver.dynids.lock);
    list_for_each_entry(dynid, &generic_pci_driver.dynids.list, node)
    {
        const struct pci_device_id id = dynid->id;
    ...
    spin_unlock(&generic_pci_driver.dynids.lock);
...
int LINUX_is_dev_in_pci_table(void *dev)
{
    int i, ret = 0;
    struct pci_dev *hwdev = (struct pci_dev *)dev;
    ...
    {
        if (pci_table[i].vendor == hwdev->vendor &&
            pci_table[i].device == hwdev->device)
        {
            ret = 1;
            goto Exit;
        }
...
unsigned long LINUX_partial_name_hash(unsigned long c, unsigned long prevHash)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return partial_name_hash(c, prevHash);
#else
    return 0;
#endif
}

unsigned long LINUX_init_name_hash(const void *salt)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
...
unsigned int LINUX_end_name_hash(unsigned long hash)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
...
const char *LINUX_kernfs_get_name(const void *pKernfsNode)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return pKernfsNode ? ((const struct kernfs_node *)pKernfsNode)->name :
        NULL;
#else
    return NULL;
#endif
...
const void *LINUX_kernfs_get_ns(const void *pKernfsNode)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return pKernfsNode ? ((const struct kernfs_node *)pKernfsNode)->ns : NULL;
#else
    return NULL;
#endif
}

unsigned int LINUX_kernfs_get_hash(const void *pKernfsNode)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return pKernfsNode ? ((const struct kernfs_node *)pKernfsNode)->hash : 0;
#else
    return 0;
#endif
}

int LINUX_kernfs_ns_enabled(void *pKernfsNode)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return kernfs_ns_enabled((struct kernfs_node *)pKernfsNode);
#else
    return 0;
#endif
}

void *LINUX_kernfs_get_parent(void *pKernfsNode)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return pKernfsNode ? ((struct kernfs_node *)pKernfsNode)->parent : NULL;
#else
    return NULL;
#endif
}

void *LINUX_kernfs_get_root_dir(void *pKernfsNode)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return pKernfsNode ? ((struct kernfs_node *)pKernfsNode)->dir.root : NULL;
#else
    return NULL;
#endif
}

void *LINUX_kernfs_get_rb_node(void *pKernfsNode)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return pKernfsNode ?
        ((struct kernfs_node *)pKernfsNode)->dir.children.rb_node : NULL;
#else
    return NULL;
#endif
}

void *LINUX_rb_node_to_kernfs_node(void *pNode)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return container_of((struct rb_node *)pNode, struct kernfs_node, rb);
#else
    return NULL;
#endif
}

void *LINUX_rb_node_get_left(void *pNode)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return pNode ? ((struct rb_node *)pNode)->rb_left : NULL;
#else
    return NULL;
#endif
}

void *LINUX_rb_node_get_right(void *pNode)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return pNode ? ((struct rb_node *)pNode)->rb_right : NULL;
#else
    return NULL;
#endif
}

void *LINUX_get_pci_driver_sd(void)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || (defined(RHEL_RELEASE) && \
    LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    struct kobject *kobj = (struct kobject *)generic_pci_driver.driver.p;

    return kobj ? kobj->sd : NULL;
#else
    return NULL;
#endif
}

void *LINUX_kernfs_root_get_rwsem(void *root)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 18, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    char *rootBytes = (char *)root;
    int pos = sizeof(struct kernfs_node *) +
        sizeof(unsigned int) +
        sizeof(struct idr) +
        2 * sizeof(u32) +
        sizeof(struct kernfs_syscall_ops *) +
    ...
    return (struct rw_semaphore *)(rootBytes + pos);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0)
    return &((struct kernfs_root *)root)->kernfs_rwsem;
#else
    return NULL;
#endif
}

void LINUX_down_read(void *sem)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
...
void *LINUX_kernfs_get_count(void *pKernfsNode)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return &((struct kernfs_node *)pKernfsNode)->count;
#else
    return NULL;
#endif
}

void *LINUX_kernfs_get_priv(void *pKernfsNode)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return ((struct kernfs_node *)pKernfsNode)->priv;
#else
    return NULL;
#endif
}

void LINUX_lockdep_assert_held(const void *l)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
...
void *LINUX_get_pci_driver_attrs(const char *pcName, int nameLen)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    const struct attribute_group **pPciDrvAttrsGroups;
    const struct attribute_group *pPciDrvAttrsGroup;
    struct attribute **pAttrs;
    struct attribute *pWantedAttr = NULL;

    if (!generic_pci_driver.driver.bus)
    {
        printk("%s: 'bus_type' is empty\n", __FUNCTION__);
        goto Exit;
    }

    pPciDrvAttrsGroups = generic_pci_driver.driver.bus->drv_groups;
    if (!pPciDrvAttrsGroups)
    {
        printk("%s: 'attribute_group' array is empty\n", __FUNCTION__);
        goto Exit;
    }

    pPciDrvAttrsGroup = pPciDrvAttrsGroups[0];
    if (!pPciDrvAttrsGroup)
    {
        printk("%s: 'attribute_group' is empty\n", __FUNCTION__);
        goto Exit;
    }

    pAttrs = pPciDrvAttrsGroup->attrs;

    for (; pAttrs && *pAttrs; pAttrs++)
    {
        if (!LINUX_strncmp((*pAttrs)->name, pcName, nameLen))
        {
            pWantedAttr = *pAttrs;
            break;
        }
...
void *LINUX_attr_to_driver_attribute(void *pAttr)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || (defined(RHEL_RELEASE) && \
    LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    return container_of((struct attribute *)pAttr, struct driver_attribute,
        attr);
#else
    return NULL;
#endif
...
void *LINUX_get_store_attribute(void *pDriverAttr)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || (defined(RHEL_RELEASE) && \
    LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    void *storeFuncAddress = 0;

    if (!pDriverAttr || !((struct driver_attribute *)pDriverAttr)->store)
        goto Exit;

    storeFuncAddress = ((struct driver_attribute *)pDriverAttr)->store;
Exit:
    return storeFuncAddress;
#else
    return 0;
#endif
}

int LINUX_run_store_attribute(void *storeFuncAddress, const char *buf,
    int count)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0) || (defined(RHEL_RELEASE) && \
    LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0))
    LINUX_STORE_FUNC storeFunc = (LINUX_STORE_FUNC)storeFuncAddress;

    return storeFunc ? storeFunc(&generic_pci_driver.driver, buf, count) : -1;
#else
    return 0;
#endif
}
...
* 0 - the current version is equal to major.minor.patch
* 1 - the current version is greater than major.minor.patch
* -1 - the current version is smaller than major.minor.patch
*/
int LINUX_compare_current_version(int major, int minor, int patch)
{
    if (LINUX_VERSION_CODE == KERNEL_VERSION(major, minor, patch))
        return 0;
    else if (LINUX_VERSION_CODE > KERNEL_VERSION(major, minor, patch))
        return 1;
    else
        return -1;
}

int LINUX_is_rhel(void)
{
    ...
    void *dev_handle, void *sglptr)
{
    ...
    struct pci_dev *hwdev = (struct pci_dev *)dev_handle;
    ...
    *dma_sglen = 0;
    ...
    #if LINUX_VERSION_CODE < KERNEL_VERSION(5,18,0)
    *dma_sglen = pci_map_sg(hwdev, sgl, page_count, (int)dma_direction);
    #else
    *dma_sglen = dma_map_sg(&hwdev->dev, sgl, page_count, (int)dma_direction);
    #endif
...
int LINUX_configure_dma_ops(void *pPciDev)
{
    int ret = -1;

#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
    struct pci_dev *hwdev = (struct pci_dev *)pPciDev;

    if (hwdev && hwdev->dev.bus && hwdev->dev.bus->dma_configure)
    {
    #if LINUX_VERSION_CODE >= KERNEL_VERSION(5,19,0) || \
        (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5,14,0))
        if (!hwdev->dev.driver)
        {
            printk("%s: Warning: DMA might not work as expected\n"
                "This Linux kernel doesn't support configuring DMA "
                "without installing a driver that takes ownership over the "
                "device. For WinDriver to take ownership you must edit "
                "'linux_wrappers.c' and add the required vid/pid to "
                "'pci_table'.\n", __FUNCTION__);
            goto Exit;
        }
    #endif
        ret = hwdev->dev.bus->dma_configure(&hwdev->dev);
    }
    #if LINUX_VERSION_CODE >= KERNEL_VERSION(5,19,0)|| \
        (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5,14,0))
        Exit:
...
#else
    ret = 0;
    printk("%s: This operation is not required in kernel version < 4.18\n"
        "In case DMA does not work as expected try taking ownership over the "
        "device. For WinDriver to take ownership you must edit "
        "'linux_wrappers.c' and add the required vid/pid to 'pci_table'.\n",
        __FUNCTION__);
#endif
    return ret;
}

void LINUX_unconfigure_dma_ops(void *pPciDev)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,19,0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5,14,0))
    struct pci_dev *hwdev = (struct pci_dev *)pPciDev;

    if (hwdev && hwdev->dev.bus && hwdev->dev.bus->dma_cleanup &&
        hwdev->dev.driver)
    {
        hwdev->dev.bus->dma_cleanup(&hwdev->dev);
    }
#endif
}
...
/* void LINUX_pci_device_set_driver(void *pdev, unsigned int *f_modify_driver)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,19,0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5,14,0))
    struct pci_dev *hwdev = (struct pci_dev *)pdev;

    if (hwdev && !hwdev->dev.driver)
    {
        hwdev->dev.driver = &generic_pci_driver.driver;

        if (f_modify_driver)
            *f_modify_driver = TRUE;
    }
#endif
}

void LINUX_pci_device_clear_driver(void *pdev, unsigned int f_modify_driver)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,19,0) || \
    (defined(RHEL_RELEASE) && LINUX_VERSION_CODE == KERNEL_VERSION(5,14,0))
    struct pci_dev *hwdev = (struct pci_dev *)pdev;

    if (hwdev && f_modify_driver)
        hwdev->dev.driver = NULL;
#endif
} */
