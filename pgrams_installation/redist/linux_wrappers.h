...
enum WD_IO_RESOURCE_TYPE
{
    WD_IO_RESOURCE_IO,
    WD_IO_RESOURCE_MEM,
    WD_IO_RESOURCE_MEM64,
};
...
typedef struct
{
    int is_vmalloced; /* Indicates if this space was allocated
                       * using vmalloc()/kmalloc() */
    void *kernel_addr;
    os_interlocked_t ref_count;
} LINUX_mmap_private;
...
#define LINUX_IS_VERSION_EQUAL(major, minor, patch) \
    LINUX_compare_current_version(major, minor, patch) == 0
#define LINUX_IS_VERSION_GREATER(major, minor, patch) \
    LINUX_compare_current_version(major, minor, patch) > 0
#define LINUX_IS_VERSION_LOWER(major, minor, patch) \
    LINUX_compare_current_version(major, minor, patch) < 0
...
int WDlinuxIoctl(struct inode *inode, struct file *filp, unsigned int cmd,
    unsigned long arg);
int wd_linux_ioctl_compat(struct inode *inode, struct file *filp,
...
long LINUX_get_time_in_millisec(void);
...
void *LINUX_get_pci_driver_attrs(const char *pcName, int size);
