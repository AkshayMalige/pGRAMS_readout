...
#include "wdusb_interface.h"
    ...
    #define DRIVER_NAME "pgrams_usb"
#else
    #define DRIVER_NAME "windrvr1630_usb"
#endif
...
#ifdef MULTIPLE_SAME_DEVICES
static struct usb_driver *g_driver = NULL;
static atomic_t g_has_device = ATOMIC_INIT(0);
#endif
...
struct usb_dev_info
{
    struct usb_device *udev; /* Stores USB device pointer */
...
{
    PVOID *addr = kmalloc(sizeof(PVOID), GFP_KERNEL);
    ...
    struct usb_endpoint_descriptor *endp)
{
    ...
    pipe_info->dwMaximumPacketSize =
        WDU_GET_MAX_PACKET_SIZE(endp->wMaxPacketSize);
...
static int calc_device_info_size(struct usb_device *udev)
{
    ...
    for (i = 0; i < udev->descriptor.bNumConfigurations; i++)
    {
        ...
        {
            struct usb_interface *interface = udev->config[i].interface[j];
            ...
            for (k = 0; k < interface->num_altsetting; k++)
            {
                ...
                count += (sizeof(WDU_ENDPOINT_DESCRIPTOR) +
                    sizeof(WDU_PIPE_INFO)) * bNumEndpoints;
            }
        }
    }
    ...
    const struct usb_device_id *id)
{
    ...
    if (!dev)
    {
        KDBG(D_ERROR, S_USB, "%s: Memory allocation failed\n", __FUNCTION__);
        ret = -ENOMEM;
        goto Exit;
    }
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_register_devices));
...
#ifdef MULTIPLE_SAME_DEVICES
    static atomic_t name_id = ATOMIC_INIT(0);
    printk("%s: g_driver[%p]\n", __FUNCTION__, g_driver);

    /* Don't create new driver if driver already exists */
    if (g_driver)
    {
        *register_ctx = (WDU_REGISTER_DEVICES_HANDLE)g_driver +
            atomic_inc_return(&name_id);
        return 0;
    }
#else
...
#ifdef MULTIPLE_SAME_DEVICES
    sprintf(name, "%s", g_cb.wd_get_driver_name());
#else
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_unregister_devices));
...
#ifdef MULTIPLE_SAME_DEVICES
    if (atomic_read(&g_has_device) != 1)
        return 0;

    driver = g_driver;
    g_driver = NULL;

    printk("%s: entered: g_driver[%p], g_has_device[%d]\n",
        __FUNCTION__, g_driver, atomic_read(&g_has_device));

#endif

    if (driver)
    {
        printk("OS_unregister_devices: Unregistering driver\n");
...
static int find_alt_set_index(struct usb_interface *iface, int alt_num)
{
    ...
    KDBG(D_ERROR, S_USB, "%s: Failed to find alternate setting [%d]",
        __FUNCTION__, alt_num);
    return -1;
}

EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_set_interface));
    ...
    WDU_ALTERNATE_SETTING **alt_setting_info, DWORD ifnum, DWORD alt_num)
{
    ...
    *alt_setting_info = NULL;
    ...
    if (!interface)
    {
        KDBG(D_ERROR, S_USB, "%s: Error- Invalid interface num selected [%d]\n",
            __FUNCTION__, ifnum);
    ...
    if (alt_index == -1)
    {
        KDBG(D_ERROR, S_USB, "%s: Error- Invalid alternate num selected [%d]\n",
            __FUNCTION__, alt_num);
        return g_cb.wd_map_error_status(-EINVAL);
    }
    ...
    if (rc)
    {
        KDBG(D_ERROR, S_USB, "%s: Failed setting interface [%d], alternate "
    ...
    altsetting_size = sizeof(WDU_ALTERNATE_SETTING) +
        (sizeof(WDU_ENDPOINT_DESCRIPTOR) + sizeof(WDU_PIPE_INFO)) * num_endp;
    ...
    if (!buf_h)
    {
        KDBG(D_ERROR, S_USB, "%s: Failed allocating interface buf memory\n",
            __FUNCTION__);
        if (buf)
            vfree(buf);
        return g_cb.wd_map_error_status(-ENOMEM);
    ...
    const WDU_ENDPOINT_DESCRIPTOR *endp)
{
    ...
    KDBG(D_TRACE, S_USB, "%s: Entered. endpoint [0x%x], type [0x%x], "
        "direction [0x%x], packet size [0x%x], interval [0x%d]\n", __FUNCTION__,
        endpoint, type, direction, endp->wMaxPacketSize, endp->bInterval);
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_open_pipe));
DWORD WD_USB_FUNC_NAME(OS_open_pipe)(HANDLE os_dev_h,
    const WDU_ENDPOINT_DESCRIPTOR *endpoint_desc, pipe_t *pipe)
{
    struct usb_dev_info *dev = os_dev_h;

    pipe->handle = (HANDLE)(unsigned long)create_usb_pipe(dev->udev,
        endpoint_desc);
    return 0;
}

EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_get_device_property));
    ...
    DWORD *buf_size, WD_DEVICE_REGISTRY_PROPERTY prop)
{
    ...
    case WdDevicePropertyLocationInformation:
    case WdDevicePropertyManufacturer:
    case WdDevicePropertyFriendlyName:
        break;
    ...
    if (actual_buf_size < needed_buf_size && int_result)
    ...
    case WdDevicePropertyLocationInformation:
        strncpy(buf, udev->devpath, actual_buf_size);
        *buf_size = strnlen(buf, actual_buf_size);
        ((CHAR *)buf)[*buf_size] = '\0';
        break;
    ...
    case WdDevicePropertyManufacturer:
        ...
        {
            strncpy(buf, udev->manufacturer, actual_buf_size);
            ((CHAR *)buf)[actual_buf_size - 1] = '\0';
        }
        *buf_size = strnlen(buf, actual_buf_size);
        break;
    ...
    case WdDevicePropertyFriendlyName:
        if (udev->product)
        {
            strncpy(buf, udev->product, actual_buf_size);
            ((CHAR *)buf)[actual_buf_size - 1] = '\0';
        }
        *buf_size = strnlen(buf, actual_buf_size);
        break;
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_get_device_info));
    ...
    DWORD active_setting, BOOL is_kernelmod, DWORD dwOptions)
{
    ...
    if (!buf)
    {
        /* Empty buffer - just return the size */
        *buf_size = calc_device_info_size(udev);
        KDBG(D_INFO, S_USB, "%s: Device info size [%ld]\n", __FUNCTION__,
            *buf_size);
        return 0;
    }
    ...
    if (!buf_h)
    {
        KDBG(D_ERROR, S_USB, "%s: Failed allocating device buf memory\n",
            __FUNCTION__);
        return g_cb.wd_map_error_status(-ENOMEM);
    }
    ...
    for (i = 0; i < udev->descriptor.bNumConfigurations; i++)
    {
        ...
        for (j = 0; j < config->bNumInterfaces; j++)
        {
            ...
            for (k = 0; k < interface->num_altsetting; k++)
            {
                ...
                pAlternateSetting->pEndpointDescriptors =
                    (WDU_ENDPOINT_DESCRIPTOR *)buf_malloc(buf_h,
                        sizeof(WDU_ENDPOINT_DESCRIPTOR) * (num_endp));
                pAlternateSetting->pPipes = (WDU_PIPE_INFO *)buf_malloc(buf_h,
                    sizeof(WDU_PIPE_INFO) * (num_endp));
    ...
    dev_buf->pActiveInterface[0]->pActiveAltSetting = pActiveAltSetting ?
        pActiveAltSetting :
        &dev_buf->pActiveInterface[0]->pAlternateSettings[0];

    return 0;
}

EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_reset_pipe));
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_halt_transfer));
...
static void urb_complete_async(struct urb *urb)
{
    ...
    {
        KDBG(D_ERROR, S_USB, "%s: Transfered [%d] more "
    ...
    {
        KDBG(D_TRACE, S_USB, "%s: Short packet. [%d] of [%d], exiting\n",
            __FUNCTION__, urb->actual_length, urb->transfer_buffer_length);
        goto urb_completed;
    }
...
static int urb_issue(struct urb_ctx *uctx)
{
    ...
    if (type == PIPE_TYPE_ISOCHRONOUS)
    {
        ...
        {
            spinlock_release(&tc->spinlock);
            KDBG(D_ERROR, S_USB, "%s: Exiting on USB_ISOCH_FULL_PACKETS_ONLY "
                "flag\n", __FUNCTION__);
            return SUCCESS_NO_NEW_URB;
        }
    }
...
#endif
{
    ...
    KDBG(D_TRACE, S_USB, "%s: Transferred [0x%x] from [0x%x] bytes. "
        "status [%d]\n", __FUNCTION__, urb->actual_length,
...
static int usb_submit_sync(struct usb_dev_info *dev, long timeout,
    int *actual_length, struct trans_ctx *tc)
{
    ...
    if (!timeout && !tc->is_finished)
    {
        ...
        {
            KDBG(D_ERROR, S_USB, "%s: Internal error. status [%d], "
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_get_max_urb_transfer_size));
    ...
    unsigned long *allocated)
{
    ...
    else if (type == PIPE_TYPE_ISOCHRONOUS)
    {
        if (!packet_size)
        {
            KDBG(D_ERROR, S_USB, "%s: Error- Can't issue transfer with non "
                "zero length and zero packet_size\n", __FUNCTION__);
            return NULL;
        }
    ...
    while (1)
    {
        ...
        {
            KDBG(D_ERROR, S_USB, "%s: Failed allocating [0x%x(%d)] bytes\n",
                __FUNCTION__, alloc_size, alloc_size);
            break;
        }
    }
...
static void uctx_put(struct urb_ctx *uctx)
{
    if (uctx->urb)
    {
        if (uctx->urb->setup_packet)
        {
            kfree(uctx->urb->setup_packet);
            uctx->urb->setup_packet = NULL;
        }
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_transfer));
DWORD WD_USB_FUNC_NAME(OS_transfer)(HANDLE os_dev_h, pipe_t *pipe, void *file_h,
    ...
    PVOID ioctl_context)
{
    ...
    *bytes_transferred = 0;
    ...
    rc = tc_create(os_dev_h, pipe, is_read, options, pl_h, bytes, setup_packet,
        tout, &tc);
    if (!tc)
        goto Exit;
    ...
    if (!bytes)
    {
        ...
        rc = usb_submit_sync(tc->dev, tc->timeout, (int *)bytes_transferred,
            tc);
        goto Exit;
    }
    ...
    if (rc)
    {
        WD_USB_FUNC_NAME(OS_halt_transfer)(tc);
        goto Exit;
    }
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_set_stream_context));
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_get_stream_context));
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_stream_request_insert));
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_is_stream_requests_queue_empty));
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_stream_transfer_create));
DWORD WD_USB_FUNC_NAME(OS_stream_transfer_create)(HANDLE os_dev_h, pipe_t *pipe)
{
    KDBG(D_ERROR, S_USB, "%s: Not supported on this platform\n", __FUNCTION__);
    return g_cb.wd_map_error_status(-EPERM);
}

EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_stream_transfer_start));
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_stream_issue_new_transfers));
void WD_USB_FUNC_NAME(OS_stream_issue_new_transfers)(void *ctx)
{
    KDBG(D_ERROR, S_USB, "%s: Not supported on this platform\n", __FUNCTION__);
}

EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_close_device));
DWORD WD_USB_FUNC_NAME(OS_close_device)(HANDLE os_dev_h)
{
    return 0;
}

EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_selective_suspend));
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_init));
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_uninit));
void WD_USB_FUNC_NAME(OS_uninit)(void)
{
#ifdef MULTIPLE_SAME_DEVICES
    printk("%s: entered\n", __FUNCTION__);
    if (g_driver && atomic_read(&g_has_device))
    {
        printk("%s: unregistering driver\n", __FUNCTION__);
        usb_deregister(g_driver);
        kfree(g_driver->name);
        kfree(g_driver->id_table);
        kfree(g_driver);
        g_driver = NULL;
    }
#else
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_wakeup));
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_reset_device));
DWORD WD_USB_FUNC_NAME(OS_reset_device)(HANDLE os_dev_h, DWORD options)
{
    struct usb_dev_info *dev = os_dev_h;
    DWORD ret = 0;

    if (usb_lock_device_for_reset(dev->udev, dev->interface))
    {
        ret = -1;
        goto Exit;
    }
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_num_pending_urbs));
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(wdusb_register_callbacks));
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_lock_user_buff));
...
EXPORT_SYMBOL(WD_USB_FUNC_NAME(OS_unlock_user_buff));
