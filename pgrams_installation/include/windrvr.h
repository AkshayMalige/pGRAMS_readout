...
#define WD_VERSION_STR_LENGTH 128
     ...
     *   the default driver name - `windrvr1630`
     *
     *   On Windows and Linux, if you select to modify the name of the
     *   WinDriver kernel module (`windrvr1630.sys/.dll/.o/.ko`),
     ...
     *                       For example, use `windrvr1630`,
     *                       not `windrvr1630.sys` or
     *                       `windrvr1630.o`.
     *
...
#elif defined(x86)
#else
#error No platform is defined. Please define either x86, x86_64, ARM or ARM64.
#endif
#if (defined(x86_64) && defined (ARM64)) || \
    (defined(x86_64) && defined (ARM)) || \
    (defined(x86_64) && defined (x86)) || \
    (defined(ARM64) && defined (x86)) || \
    (defined(ARM64) && defined (ARM)) || \
    (defined(x86) && defined(ARM)) 
#error Cannot have more than one platform defined in a WinDriver compilation.
#endif
...
#if defined(__KERNEL__) && defined(WINNT)
    #if (NTDDI_VERSION >= NTDDI_WIN10_VB)
        #define WD_VER_STR_SUFFIX ""
    #else
        #define WD_VER_STR_SUFFIX " for Windows 10 (Build 18632 and Lower)"
    #endif
#else
    #define WD_VER_STR_SUFFIX ""
#endif
...
#define WD_VER_STR  WD_PROD_NAME " v" WD_VERSION_STR \
    " Jungo Connectivity (c) 1997 - " COPYRIGHTS_YEAR_STR \
        " Build Date: " __DATE__ \
    WD_CPU_SPEC WD_DATA_MODEL WD_FILE_FORMAT WD_VER_STR_SUFFIX
...
typedef enum {
    ...
    DMA_KBUF_ALLOC_SPECIFY_ADDRESS_WIDTH = 0x800, /**< When using this flag, the
        ...
        This flag is not supported on Windows ARM64. */
...
typedef struct
{
    ...
    PAD_TO_64(hKernelPlugIn) /* 64 bit app has a 4 byte hole here */
    PVOID pOpenData;
    PAD_TO_64(pOpenData)
...
#define IOCTL_WD_DMA_TRANSACTION_RELEASE            WD_CTL_CODE(0xa01)
    ...
    DWORD DLLCALLCONV check_secureBoot_enabled(void);
        ...
        #define WD_UStreamWrite(hFile, pBuffer, dwNumberOfBytesToWrite, \
            ...
            #define IO_MASTER_PORT IOMainPort
        #else
            #define IO_MASTER_PORT IOMasterPort
        #endif
...
* 
* 4. WD_CardRegister() will only work with the first item being defined as an
*    ITEM_BUS (see example below).
*
