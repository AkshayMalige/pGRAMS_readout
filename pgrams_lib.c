...
#define PGRAMS_DEFAULT_LICENSE_STRING "12345abcde12345.abcde"

#define PGRAMS_DEFAULT_DRIVER_NAME WD_DEFAULT_DRIVER_NAME_BASE

/* PCI device information struct */
typedef struct {
    
    
    PGRAMS_INT_HANDLER   funcDiagIntHandler;   /* Interrupt handler routine */
    PGRAMS_EVENT_HANDLER funcDiagEventHandler; /* Event handler routine */

} PGRAMS_DEV_CTX, *PPGRAMS_DEV_CTX;
...
static CHAR gsPGRAMS_LastErr[256];
 ...
 *************************************************************/
    ...
    static void PGRAMS_EventHandler(WD_EVENT *pEvent, PVOID pData);
#endif
static void DLLCALLCONV PGRAMS_IntHandler(PVOID pData);
...
{
    if (!pDev || !(PPGRAMS_DEV_CTX)(pDev->pCtx))
    {
        snprintf(gsPGRAMS_LastErr, sizeof(gsPGRAMS_LastErr) - 1,
            "%s: NULL device %s\n", sFunc, !pDev ? "handle" : "context");
        ErrLog(gsPGRAMS_LastErr);
        return FALSE;
    }
...
DWORD PGRAMS_LibInit(void)
{
    ...
    /* Set the driver name */
    if (!WD_DriverName(PGRAMS_DEFAULT_DRIVER_NAME))
    {
    ...
    dwStatus = WDC_DriverOpen(WDC_DRV_OPEN_DEFAULT, PGRAMS_DEFAULT_LICENSE_STRING);
    if (WD_STATUS_SUCCESS != dwStatus)
    {
...
WDC_DEVICE_HANDLE PGRAMS_DeviceOpen(DWORD dwVendorId, DWORD dwDeviceId)
{
    ...
    hDev = WDC_DIAG_DeviceFindAndOpen(dwVendorId,
        dwDeviceId, KP_PGRAMS_DRIVER_NAME, sizeof(PGRAMS_DEV_CTX));
...
Error:
    if (hDev)
        PGRAMS_DeviceClose(hDev);

    ErrLog("PGRAMS_DeviceOpen: Failed opening PCI device\n");
...
{
    TraceLog("PGRAMS_DeviceClose: Entered. Device handle 0x%p\n", hDev);

    /* Disable interrupts (if enabled) */
    if (WDC_IntIsEnabled(hDev))
    {
        DWORD dwStatus = PGRAMS_IntDisable(hDev);
        if (WD_STATUS_SUCCESS != dwStatus)
        {
...
static void DLLCALLCONV PGRAMS_IntHandler(PVOID pData)
{
    ...
    PPGRAMS_DEV_CTX pDevCtx = (PPGRAMS_DEV_CTX)(pDev->pCtx);
    PGRAMS_INT_RESULT intResult;
...
DWORD PGRAMS_IntEnable(WDC_DEVICE_HANDLE hDev, PGRAMS_INT_HANDLER funcIntHandler)
{
    ...
    PPGRAMS_DEV_CTX pDevCtx;

    
    

    TraceLog("PGRAMS_IntEnable: Entered. Device handle 0x%p\n", hDev);

    /* Validate the device handle */
    if (!IsValidDevice(pDev, "PGRAMS_IntEnable"))
    ...
    pDevCtx = (PPGRAMS_DEV_CTX)(pDev->pCtx);
    ...
    /* Store the diag interrupt handler routine, which will be executed by
       PGRAMS_IntHandler() when an interrupt is received */
    ...
    dwStatus = WDC_IntEnable(hDev, NULL, 0, 0,
        PGRAMS_IntHandler, (PVOID)pDev, WDC_IS_KP(hDev));
    ...
    TraceLog("PGRAMS_IntEnable: Interrupts enabled\n");
...
DWORD PGRAMS_IntDisable(WDC_DEVICE_HANDLE hDev)
{
    ...
    TraceLog("PGRAMS_IntDisable entered. Device handle 0x%p\n", hDev);

    /* Validate the device handle */
    if (!IsValidDevice(pDev, "PGRAMS_IntDisable"))
        return WD_INVALID_PARAMETER;
...
{
    /* Validate the device handle */
    if (!IsValidDevice((PWDC_DEVICE)hDev, "PGRAMS_IntIsEnabled"))
        return FALSE;
...
{
    PWDC_DEVICE pDev = (PWDC_DEVICE)hDev;
    TraceLog("PGRAMS_SriovEnable entered. Device handle: 0x%p\n", hDev);

    /* Validate the device handle */
    if (!IsValidDevice(pDev, "PGRAMS_SriovEnable"))
        return WD_INVALID_PARAMETER;
...
DWORD PGRAMS_SriovDisable(WDC_DEVICE_HANDLE hDev)
{
    ...
    TraceLog("PGRAMS_SriovDisable entered. Device handle: 0x%p\n", hDev);

    /* Validate the device handle */
    if (!IsValidDevice(pDev, "PGRAMS_SriovDisable"))
        return WD_INVALID_PARAMETER;
...
DWORD PGRAMS_SriovGetNumVFs(WDC_DEVICE_HANDLE hDev, DWORD* pdwNumVFs)
{
    ...
    TraceLog("PGRAMS_SriovGetNumVFs entered. Device handle: 0x%p\n", hDev);
    if (pdwNumVFs)
        *pdwNumVFs = 0;

    /* Validate the device handle */
    if (!IsValidDevice(pDev, "PGRAMS_SriovGetNumVfs"))
        return WD_INVALID_PARAMETER;
...
static void PGRAMS_EventHandler(WD_EVENT *pEvent, PVOID pData)
{
    ...
    TraceLog("PGRAMS_EventHandler entered, pData 0x%p, dwAction 0x%x\n", pData,
        pEvent->dwAction);
    ...
    PGRAMS_EVENT_HANDLER funcEventHandler)
{
    ...
    TraceLog("PGRAMS_EventRegister entered. Device handle 0x%p\n", hDev);

    /* Validate the device handle */
    if (!IsValidDevice(pDev, "PGRAMS_EventRegister"))
        return WD_INVALID_PARAMETER;
     ...
     * PGRAMS_EventHandler() upon an event */
    pDevCtx->funcDiagEventHandler = funcEventHandler;

    /* Register the event */
    dwStatus = WDC_EventRegister(hDev, dwActions, PGRAMS_EventHandler, hDev,
        WDC_IS_KP(hDev));
...
DWORD PGRAMS_EventUnregister(WDC_DEVICE_HANDLE hDev)
{
    ...
    TraceLog("PGRAMS_EventUnregister entered. Device handle 0x%p\n", hDev);

    /* Validate the device handle */
    if (!IsValidDevice((PWDC_DEVICE)hDev, "PGRAMS_EventUnregister"))
        return WD_INVALID_PARAMETER;
...
{
    /* Validate the device handle */
    if (!IsValidDevice((PWDC_DEVICE)hDev, "PGRAMS_EventIsRegistered"))
        return FALSE;
...
DWORD PGRAMS_GetNumAddrSpaces(WDC_DEVICE_HANDLE hDev)
{
    ...
    /* Validate the device handle */
    if (!IsValidDevice(pDev, "PGRAMS_GetNumAddrSpaces"))
        return 0;
    ...
    PGRAMS_ADDR_SPACE_INFO *pAddrSpaceInfo)
{
    ...
    if (!IsValidDevice(pDev, "PGRAMS_GetAddrSpaceInfo"))
        return FALSE;

#if defined(DEBUG)
    if (!pAddrSpaceInfo)
    {
        ErrLog("PGRAMS_GetAddrSpaceInfo: Error - NULL address space information pointer\n");
        return FALSE;
    }
#endif
    ...
    {
        ErrLog("PGRAMS_GetAddrSpaceInfo: Error - Address space %d is "
...
static void ErrLog(const CHAR *sFormat, ...)
{
    ...
    vsnprintf(gsPGRAMS_LastErr, sizeof(gsPGRAMS_LastErr) - 1, sFormat, argp);
#ifdef DEBUG
        WDC_Err("PGRAMS lib: %s", gsPGRAMS_LastErr);
#endif
    va_end(argp);
}
